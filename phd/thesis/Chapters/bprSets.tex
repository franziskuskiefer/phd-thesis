...
\begin{enumerate}
 ...
 % \item Server Privacy: Client $C$ learns nothing new about the server sets $\ServerSet$.
...
\matrix (m)[matrix of nodes, column  sep=.1cm,row  sep=1mm, ampersand replacement=\&,
	...
	\parbox{18em}{get shares from $G_{\ClientSet\cap\ServerSet}$: $\{\secret_i|\rho(i)\in \ServerSet\cap \ClientSet\}$} \& % generate $B_{\ClientSet}$ encoding $\ClientSet$ generate $G_{\ServerSet}$ encoding $\ServerSet$ and $\{\secret_{\rho(s_i)}: s_i\in \ServerSet\}$
	  \ac{OBI} \& \\
% 	\parbox{18em}{query $G'$ with all $\ClientSet$ to get according $s_{xi}$} \& \& \\
...
\matrix (m)[matrix of nodes, column  sep=.1cm,row  sep=1mm, ampersand replacement=\&,
	...
	\parbox{18em}{$\ClientSet=\{c_1,c_2,\ldots,c_v\}\gets\psi(\pwd)$} \&  \&  \\
