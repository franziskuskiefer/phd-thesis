...
The Oblivious Bloom Intersection (OBI) protocol by Dong et al. \cite{DongCW13} is executed between a client and a server on the respective sets \cC and \cS. 
...
As observed in \cite{WenD14}, OBI can be extended to a Private Set Intersection with Data Transfer protocol. In this case, the server can associate each element $s_i\in\cS$ with a data item $d_i$. 
...
The protocol can be described at a high level as follows: let the server hold a set $\cS=\{s_i\}$ and a data set $\cS_d=\{d_{i}\}$. The two sets are of equal cardinality and each $(s_i,d_i)$ can be viewed as a key-value pair. The server generates a garbled Bloom filter $G_\cS$ on \cS and $\cS_d$ using \cite[Algorithm 1]{WenD14}. 
...
In this paper we use OBI so that the server can send a set of secret shares to the client based on the client's set $\cC$ without knowing anything about $\cC$. Although in general we can use any PSI with Data Transfer protocol (e.g. \cite{fre04}), we choose OBI here because of its efficiency. OBI is very efficient due to the fact that it relies mostly on hash operations. The performance can be further improved by the modifications proposed by Pinkas et al. \cite{Pinkas0Z14}. Note that although Pinkas et al. also proposed a new PSI protocol based on hashtable + oblivious transfer in \cite{Pinkas0Z14} that is more efficient than OBI, the new PSI protocol cannot be used in our case because it does not support data transfer.
...
Security then follows from the composition theorem \cite{Canetti00}.
...
as shown by Canetti \cite{Canetti00}.
