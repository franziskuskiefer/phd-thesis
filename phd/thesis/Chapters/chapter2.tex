...
When saying something is hard with respect to $\secpar$, this means that there exists no algorithm that can solve the problem in polynomial time in $\secpar$.
...
The factoring problem in $\ZZ^\ast_N$ states that given modulus $N$ it is hard with respect to $\secpar$ to compute $p',q'$ such that $N=p'q'$.
\eod
\end{definition}

\noindent
To define RSA-based assumptions we have to define the RSA algorithm first.
...
The RSA problem over $\ZZ^\ast_N$ states that given $(N,e,d,y)$ with $\gcd(e, \phi(N))=1$ and $ed=1\mod \phi(N)$ it is hard with respect to $\secpar$ to compute $x$ such that $x^e = y \mod N$.
\eod
\end{definition}
...
\mynote{deinfe $\varepsilon$-closeness}
...
This section describes all building blocks used throughout this work to build the proposed protocols.
But before starting with that we want to give a short description of the random oracle, which is an important though disputed, and standard model.
...
While not including random oracles, the standard model often includes a \acl{CRS} that is made available to all parties and can be seen as common set-up parameters.
...
\mynote{define how we write oracles and restrictions on the number of invocations (define earlier)}
...
\hspace*{2em} $b'\gets\cA^{\cO^1_{\PKGen}(\cdot), \cO^1_{\Hash_b}(\cdot)}(\secpar, \crs, \crs', \tau)$ \\
...
\noindent
On input \pwd the $\cO_{\PKGen}$ oracle draws a new hash key \hk for \ac{T-SPHF} on \Lpwd, computes the according projection key \hp using \PKGen, and returns it to the adversary.
