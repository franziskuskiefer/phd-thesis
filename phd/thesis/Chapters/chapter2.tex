...
\begin{itemize}
  ...
  \item Type II: $\GG_1 \not= \GG_2$ and there exists an efficiently computable homomorphism $\phi: \GG_2 \mapsto \GG_1$ (but not for $\GG_1 \mapsto \GG_2$);
...
Asymptotic notation allows us to describe the behaviour of a function when its arguments converge towards some limit.
...
In our demo we use the tSoke protocol proposed by \citet{Manulis2014} as authentication protocol for convenience.
It is recalled in Figure \ref{fig:tSOKE}.
First note that the description in Figure \ref{fig:tSOKE} omits session key generation and describes a \ac{tPAuth} protocol.
Further note that while the original tSOKE protocol is proven to be a secure \ac{PAKE} protocol, it is not a secure \ac{VPAKE} protocol.
...
$x \rin \ZZ_p$ & & \\
$X = g^x$ & & \\
& $\xrightarrow{\makebox[2cm]{$\Client, X$}}$ & \\
& & Look up \ver for $\Client$. \\
& & $y \rin \ZZ_p$ \\
& & $Y = g^y$ \\
& & $Y^{*} = Y \cdot h^{H}$ for $\ver=(H,r)$ \\
& $\xleftarrow{\makebox[2cm]{$r, Y^{*}$}}$ & \\
$H \gets \HashP(\pwd, r)$ & & \\
$Y = Y^{*} / h^H$ & & \\
$Z = Y^x$ & & $Z' = X^y$ \\
$K \gets \SHA(\Client, H, \tg, X, Y^{*}, Z)$ & & $K' \gets \SHA(\Client, H, \tg, X, Y^{*}, Z)$ \\
$A_{1} \gets \SHA(K , \mbox{``auth1''})$ & & \\
& $\xrightarrow{\makebox[2cm]{$A_{1}$}}$ & \\
& & Abort if $A_{1} \ne \SHA(K', \mbox{``auth1''})$ \\
& & $A_{2} \gets \SHA(K' , \mbox{``auth2''})$ \\
& $\xleftarrow{\makebox[2cm]{$A_{2}$}}$ & \\
Abort if $A_{2} \ne \SHA(K, \mbox{``auth2''})$ & & \\
Accept & & Accept \\
\bottomrule
\end{tabular}
...
A main goal of (cryptographic) hash functions is to be as collision-resistant as possible.
...
Pedersen commitments are \emph{additively homomorphic}, \ie for all $(C_i,d_i) \gets \Com(x_i;r_i)$ for $i\in 1,\dots,n$ it holds that $\prod_{i=1}^{n}C_i=\Com(\sum_{i=1}^{n}x_i$; $\sum_{i=1}^{n}r_i)$.
...
A zero-knowledge proof is executed between a prover and a verifier, proving that a word $x$ is in a language $L$, using a witness $w$ proving so, \ie there exists $w$ such that $(x,w)\in R$ for relation $R$.
...
As a result, the client receives a garbled Bloom filter $G_{\ClientSet\cap\ServerSet}$ that encodes the intersection $\ClientSet\cap \ServerSet$ and the data items associated with the elements in $\ClientSet\cap \ServerSet$ after successfully resembling \sk, decrypting $c_i$ and verifying $r_i$. 
...
\toprule
{\bf Client \Client} & & {\bf Server \Server} \\
Input: BF $B_\ClientSet$, Set \ClientSet & & Input: GBF $G_\ServerSet$ \\
\midrule
& neg. params & \\
& $(n, m, k, H, E)$ & \\
for $i\in[0,m-1]$ & & \\ [-.25em]
...
& & $\forall i$ $\in$ $[0,m-1]$ \\
& & \hspace*{1em} $c_i$ $\gets$ $\Enc_\sk(r_i||G_\ServerSet[i])$ \\
...
Build $G_{\ClientSet\cap \ServerSet}$ with Algorithm \ref{alg:gbfintersection} & & \\
\hspace*{1em} on $(\{d_i\}, B_\ClientSet, m, \lambda)$ & & \\
Query $G_{\ClientSet\cap \ServerSet}$ with \ClientSet & & \\
\hspace*{1em} to retrieve $\ClientSet\cap \ServerSet$ & &\\
\bottomrule
\end{tabular}
}
\end{center}
...
\FOR{$i=0$ \TO $|\ServerSet|-1$}
  ...
  \FOR{$j=0$ \TO $k-1$}
    ...
    \ENDIF
  \ENDFOR
  \STATE GBF$_\ServerSet[\text{emptySlot}]=\share$
\ENDFOR
\FOR{$i=0$ \TO $m-1$}
