...
\mynote{some words on composition and \ac{UC}}

\subsection{Universal Composability}
\mynote{something?}

To implement our 2PAKE functionality we require some commonly used functionality from the literature.
First, since we work in the crs model we require the crs functionality from \cite{CanettiF01}, recalled in Figure \ref{fig:crsfunctionality}.
We further need verified public keys on both servers.
We use the ideal CA functionality \Fca from \cite{Canetti04} for this, recalled in Figure \ref{fig:cafunctionality}.
Eventually, to establish unique query identifiers between the parties in a protocol run we use the \Finit functionality from \cite{BarakLR04}, recalled in Figure \ref{fig:initfunctionality}.
...
\mynote{BP VPAKE model}

\subsection{Two-server PAKE Security}
\mynote{Katz 2PAKE model}
...
% We recall \cite[Algorithm 1]{LiuC10} to generate \ac{LSSS} matrix from a boolean formula in Algorithm \ref{alg:lsssM} for completeness.
