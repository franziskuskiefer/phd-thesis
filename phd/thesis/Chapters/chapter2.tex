...
% Two protocol participants $P,P'$ are compatible if w.l.o.g. $P\in\Ciphertexts$ and $P'\in\cS$, and $\PHash(\paramP, \PPreHash(\paramP, \pwd_P, s_P), s_H) = H_{P'}$.
...
& Input: tag $\tg$ & & Input: tag $\tg$ \\
1. & Enter username $\id_{\Client}$. & & \\
2. & Enter password $\pwd_{\Client,\Server}$. & & \\
3. & $x \rin \{2, \dots, n-1\}$ & & \\
4. & $X = g^x$ & & \\
5. & & $\stackrel{\id_{\Client}, X}{\llongrightarrow}$ & \\
6. & & & Look up \ver for $\id_{\Client}$. \\
7. & & & $y \rin \{2, \dots, n-1\}$ \\
8. & & & $Y = g^y$ \\
9. & & & $Y^{*} = Y \cdot h^{H}$ for $\ver=(H,r)$ \\
10. & & $\stackrel{r, Y^{*}}{\llongleftarrow}$ & \\
11. & $H \gets \HashP(\pwd, r)$ & & \\
12. & $Y = Y^{*} / h^H$ & & \\
13. & $Z = Y^x$ & & $Z' = X^y$ \\
14. & $K \gets \SHA(\id_{\Client}, H, \tg, X, Y^{*}, Z)$ & & $K' \gets \SHA(\id_{\Client}, H, \tg, X, Y^{*}, Z)$ \\
15. & $A_{1} \gets \SHA(K , \mbox{``auth1''})$ & & \\
16. & & $\stackrel{A_{1}}{\llongrightarrow}$ & \\
17. & & & Abort if $A_{1} \ne \SHA(K', \mbox{``auth1''})$ \\
18. & & & $A_{2} \gets \SHA(K' , \mbox{``auth2''})$ \\
19. & & $\stackrel{A_{2}}{\llongleftarrow}$ & \\
20. & Abort if $A_{2} \ne \SHA(K, \mbox{``auth2''})$ & & \\
21. & Accept & & Accept \\
\bottomrule
\end{tabular}
...
\end{center}
\caption[The tSoke Protocol]{The tSoke Protocol \citet{Manulis2014}}
\label{fig:tSOKE}
\end{figure*}
...
& $(n, m, k, H, E)$ & \\
for $i\in[0,m-1]$ & & \\ [-.25em]
\hspace*{1em} gen.  $r_i\ralgout\{0,1\}^\lambda$ & $\stackrel{\{r_i\}_{i\in[0,m-1]}}{\llongrightarrow}$ & $\sk$ $\ralgout$ $\KGen_E(\lambda)$ \\
& & $\forall i$ $\in$ $[0,m-1]$ \\
& & \hspace*{1em} $c_i$ $\gets$ $\Enc_\sk(r_i||G[i])$ \\
& & share \sk into $t_0, \dots, t_{m-1}$ ($\oplus$) \\
& {\bf OT} on& \\
Recover $t_i$ if $B_\ClientSet[i]=0$ & $B$ and $\{(c_i, t_i)\}_{i\in[0,m-1]}$ & \\
Recover \sk from $t_i$ & & \\
