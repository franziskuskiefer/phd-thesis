...
\mynote{rewrite below when BPR is implemented}
...
As we can see in the figure, the time for computing $u_i$ and $u_i'$ is linear in the password length, and the time for computing $\hat{\cS}$ and executing \ac{SPC} is linear in the size of $\hat{\cS}$. 
...
Since the computation of $\hat{S}$ does not depends on the client's password, the server can generate a random \ac{RSA} key pair and pre-compute $\hat{S}$ before engaging with the client. 
...
If this step is taken offline, then the online computation cost is small, usually no more than 100 milliseconds in a typical setting. 
