...
At the end of the protocol the server eventually outputs the password verifier $\ver$ for a policy compliant, client chosen password \pwd. %for a suitable \ac{VPAKE} protocol without learning anything about the password in the verifier.
\eod
\end{definition}
...
%   \item The password verifier $\ver$ for a client identified by the client identifier \Client is stored on the server.
...
Using $\cD_{f,n}$ in the definition of \ac{DAR} seems a reasonable compromise between the desired security and efficiency for \ac{BPR} protocols.
...
For remote registration of the password verifier we assume that the \ac{BPR} protocol is executed over a server-authenticated secure channel in order to protect transmission of $\ver$; otherwise an eavesdropping adversary would be able to recover the password by brute-forcing the dictionary. 
...
% 	\& \& \hspace*{2em} store $(\Client,\ver)$ with $\ver=(H_1,H_2,s_H)$ \\
% };
%
...
Execute \PoE, \PoM, \PoS: & $\bm C, \bm C', \vect{\omega}, H_1,H_2, s_H,$ & If $|\bm C|$ $=$ $|\bm C'|$ $\geq$ $\pmin$: \\
 & $\xrightarrow{\makebox[4cm]{$\Comm_{\PoM}, \Comm_{\PoE}, \Comm_{\PoS}$}}$ & \hspace*{1em} choose $\Ch_\PoM, \Ch_\PoE, \Ch_\PoS$ \\
...
Let $\Ch_0=1$.
\[ s_v = \sum_{j=0}^{n} A_{vj}\Ch_j;~ s'_v = A'_v + \sum_{j=1}^{n} A_{vj}\Ch_j^2 \] % \text{ for } v\in[-4,n]
...
\subsection{Discussion} \label{sec:bpr-discussion}
