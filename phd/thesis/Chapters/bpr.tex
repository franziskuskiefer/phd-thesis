...
At the end of the protocol the server eventually outputs the password verifier $v_C$ for a policy compliant, client chosen password \pwd. %for a suitable \ac{VPAKE} protocol without learning anything about the password in the verifier.
\eod
\end{definition}
...
Instances of protocol participants \Client or \Server are denoted $\Client_i$ or $\Server_i$. %, where $i$ is modelled as a counter to ensure uniqueness.
...
%   \item The password verifier $v_C$ for a client identified by the client identifier \Client is stored on the server.
