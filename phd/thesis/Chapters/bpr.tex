\section{Blind Password Registration}
...
We discuss \ac{VPAKE} protocols that can be used with verifiers set-up with this \ac{BPR} protocol in Section \ref{sec:vpake}.
Blind password registration is formally defined as follows.

\begin{definition}[Blind Password Registration]\label{def:bpr}
A BPR protocol is executed between a client $C$ and a server $S$ with server's password policy $f$ as a common input. At the end of the protocol the server eventually outputs the password verifier $v_C$ for a policy compliant, client chosen password \pwd. %for a suitable VPAKE protocol without learning anything about the password in the verifier.
\end{definition}

\subsection{Security of Blind Password Registration} \label{sec:bpr-model}
...
Instances of protocol participants $C$ or $S$ are denoted $C_i$ or $S_i$. %, where $i$ is modelled as a counter to ensure uniqueness.
...
%   \item The password verifier $v_C$ for a client identified by the client identifier $C$ is stored on the server.
...
\noindent
% Security of a BPR protocol is defined by two properties: \emph{policy compliance} and \emph{dictionary attack resistance}.
...
% This leads to the following definition of BPR security in the semi-honest server model.

% \begin{definition}[BPR Security]\label{def:bprsecurity}
% A BPR protocol is secure with respect to password hashing scheme \Hash in the semi-honest server model if it offers policy compliance according to Definition \ref{def:pc} and dictionary attack resistance according to Definition \ref{def:dar}.
% \end{definition}
...
%===============================================================================
% Section: Efficient BPR Construction
%===============================================================================
...
% \begin{figure}[!h]
%   \centering
%     \fbox{\parbox[b][][t]{0.7\textwidth}{
%     \begin{itemize}
%       \item Choose $\pwd\rin\cD_f$
% %       \item Encode $\pi\gets\Encode(\pwd)$
%       \item Commit to all characters in \pwd: \\
%         \hspace*{2em} $C_i\gets \Com(\pi_i;r_i)=g^{\pi_i}h^{r_i},~ C'_i=C_ih^{r'_i}$
%       \item Shuffle $\bm C'$
%       \item Compute password hash \\
%         \hspace*{2em} $(H_1,H_2)\gets\HashP(\pi)=(g^{s_P}, H_1^{\pi}h^{s_H})$
%       \item \PoM: set membership proofs for $C'_i$ and $\Sigma$ or $R_j$ if significant
%       \item \PoE: $H_2$ and $\prod C^{95^i}_i$ contain the same password $\pi$
%       \item \PoS: $\bm C'$ is a shuffle of $\bm C$
%     \end{itemize}} }
% %   \end{mdframed}
%   \caption{BPR Protocol --- An Overview} %\label{fig:protocol-overview}
% \end{figure}
...
\node[party] (client) at (1,0.5) {\uline{Client $(C,S,f)$}};

\node[state, align=left] at (1,0) [stateS, align=left]{Choose $\pwd=(c_0,\dots,c_{n-1})\in\cD_f$; compute $\pi\gets\pwdint(\pwd)$;};
\node[state, align=left] at (1.0,-0.4) [stateS, align=left]{For all $i\in[0,n-1]$ compute:};
\node[state, align=left] at (1.25,-0.7) [stateS, align=left]{$\pi_i\gets\chrint(c_{i})$; $C_i=g^{\pi_i}h^{r_i};~ C'_i=C_ih^{r'_i}$;};
\node[state, align=left] at (1.0,-1.2) [stateS, align=left]{Shuffle $C'_i \gets C'_{k_i}$ and define sets $\bm C = \{C_i\}$, $\bm C' = \{C'_i\}$};
\node[state, align=left] at (1,-1.6) [stateS, align=left]{Compute the randomised password hash:};
\node[state, align=left] at (1.25,-2.0) [stateS, align=left]{$(H_1,H_2)\gets\HashP(\pi)=(g^{s_P}, H_1^{\pi}h^{s_H})$};

\node[state, align=left] at (1,-2.6) [stateS, align=left]{For each $c_i\in\pwd$ identify appropriate set $\omega_{k_i}$;}; %denote a $R_j$ if $c_i$ is significant, $\Sigma$ otherwise\smallskip};
\node[state, align=left] at (1,-3.0) [stateS, align=left]{Execute the following protocols with the server:};
\node[state, align=left] at (1,-3.8) [stateS, align=left]{\PoE: $\ZKPoK\{(\pi,\hat{r}) : ~ H_2/h^{s_H}=H_1^{\pi} \wedge \prod_{i=0}^{n-1} C_i^{b^i} = g^{\pi}h^{\hat{r}}\}$};
\node[state, align=left] at (1,-4.4) [stateS, align=left]{\PoM: $\ZKPoK\{\{\pi_i,r_i\}_{i} : ~ C'_{k_i}=g^{\pi_i}h^{r_i} \wedge \pi_i\in \omega_{k_i}\}$};
\node[state, align=left] at (1,-5.0) [stateS, align=left]{\PoS: $\ZKPoK\{\{k_i, r'_{k_i}\}_{i} : ~ C'_{i}=C_{k_i}h^{r'_{k_i}}\}$};

\node[dummyState] (clientCom) at (5.0,-3.8){};
\node[dummyState] (clientCh) at (5.0,-4.4){};
\node[dummyState] (clientRes) at (5.0,-5.0){};

\node[party,align=center,text width=11em] (server) at (11.5,0.5) {\uline{Server $(C,S,f)$}};

\node[state, align=left] at (11.5,-3.6) [stateSmS, align=left]{};
\node[state, align=left] at (11.5,-3.8) [stateSmS, align=left]{If $|\bm C|=|\bm C'| \geq \pmin$:};
\node[state, align=left] at (11.9,-4.2) [stateSmS, align=left, text width=10em]{choose};
\node[state, align=left] at (11.9,-4.6) [stateSmS, align=left, text width=10em]{$\Ch_\PoM, \Ch_\PoM, \Ch_\PoS$};
\node[state, align=left] at (11.5,-5.2) [stateSmS, align=left]{If $\PoM$, $\PoE$, $\PoS$};
\node[state, align=left] at (11.5,-5.6) [stateSmS, align=left]{succeed, store $(C,v_C)$};
\node[state, align=left] at (11.5,-6.0) [stateSmS, align=left]{with $v_C=(H_1,H_2,s_H)$};

\node[dummyState] (serverCom) at (9.5,-3.8){};
\node[dummyState] (serverCh) at (9.5,-4.4){};
\node[dummyState] (serverRes) at (9.5,-5.0){};

\draw[pil] (clientCom) -- node[above, align=center] {$\bm C, \bm C', \vect{\omega}, H_1,H_2, s_H,$\\ $\Comm_{\PoM}, \Comm_{\PoE}, \Comm_{\PoS}$} (serverCom);
\draw[pil] (serverCh) -- node[above, align=center] {$\Ch_{\PoM}, \Ch_{\PoE}, \Ch_{\PoS}$} (clientCh);
\draw[pil] (clientRes) -- node[above, align=center] {$\Res_{\PoM}, \Res_{\PoE}, \Res_{\PoS}$} (serverRes);
\end{tikzpicture}}
...
% For better readability we move description of \PoS to a separate section below and denote its messages by \PoSm.
% Figure \ref{fig:protocol} shows the exchanged messages.

% \begin{figure}[htbp]
%   \centering
%   \begin{tabular}{|lll|}
%     \hline
%     1) & $C \rightarrow S:$ & $(\bm C, \bm C', \vect{\omega}, H_1,H_2, s_H, (\bm t), (t_{s_P}, t_H, t_{C^\ast}), \PoSm)$ \\
%     2) & $S \rightarrow C:$ & $(c, \PoSm)$ \\
%     3) & $C \rightarrow S:$ & $((\bm s, \bm c), (s_{s_P}, s_\pi, s_{r^\ast}), \PoSm)$ \\
%     \hline
%   \end{tabular}
%   \caption{BPR Protocol Messages} \label{fig:protocol}
% \end{figure}
...
% commit to $\pi_i$ by computing Pedersen commitments $C_i=g^{\pi_i}h^{r_i}, C'_i=C_i h^{r'_i}$ for $r_i,r'_i\rin\Zrq$;
...
% The \PoS protocol is in particular an honest verifier zero-knowledge proof of knowledge of the following statement if the discrete logarithm problem in the used group is hard,
...
\subsubsection{Proof of Equivalence (\PoE)}
...
\end{enumerate}
%, where $c$ is the challenge chosen for \PoM. %, checks them for group membership
...
\begin{enumerate}
  ...
  \item To verify the proof, i.e. to verify that every commitment $C'_{k_i}$ in $\bm C'$ commits to a character $c_i$ from either a subset $R_j$ of $\Sigma$ if significant or $\Sigma$ if not, the server verifies the following for every set $\omega_{i} \in \vect{\omega}$ with $i\in[1,n]$: % and $x=k_i$:
    \begin{itemize}
...
The matrix $A_{ji}$ is defined with $A_{w0}\rin\Zrq, A_{-1v}\rin\Zrq$ and $A_{0v}=r'_{\phi(v)}$ with $w\in[-4,n]$ and $v\in[1,n]$. %depicted in Figure \ref{fig:matrix}
...
\noindent
After generating $A_{ji}$ the client commits to it and sets his output $\Comm_\PoS=(C'_0, \tilde{f}, \bm f', w, \tilde{w})$ for $\bm f'=\{f'_v\}$ with $v\in[0,n]$.
\[ f'_v=\prod_{j=-4}^{n} f_j^{A_{jv}};~ \tilde{f}=\prod_{j=-4}^{n} f_j^{A'_{j}} \] %  \text{ for } v\in[0,n]
...
%       \item Choose random values $A_{w0},A_{-1v},A'_{w0}\rin \Zrq$ for $w\in[-4,n]$ and $v\in[1,n]$
...
\item The server chooses $\bm c = \{c_v\}$ with $c_v\rin\Zrq$ for $v\in[1,n]$ and sets $\Ch_\PoS=\bm c$.
...
% \noindent
% The proof of Theorem~\ref{theo:bpr} follows then from the following Lemmas \ref{lem:pc} and  \ref{lem:dar} (with corresponding proofs in Appendices \ref{app:proof-pc} and \ref{app:proof-dar}) that separately prove each of the two BPR security requirements, namely policy compliance and dictionary attack resistance.
% 
% 
% \begin{lemma} \label{lem:pc}
%   The BPR protocol from Section \ref{sec:protocol} offers policy compliance according to Definition \ref{def:pc} if the discrete logarithm problem is hard in the used group.
% \end{lemma}
% 
% \begin{lemma} \label{lem:dar}
%   The BPR protocol from Section \ref{sec:protocol} offers dictionary attack resistance according to Definition \ref{def:dar} if the discrete logarithm problem is hard in the used group.
% \end{lemma}
