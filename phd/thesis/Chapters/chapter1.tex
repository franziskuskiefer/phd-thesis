...
\mynote{check password hashing competition for final version when done pwdcon'15 Vegas}
...
\subsubsection{Game-Based \ac{PAKE}-Security}
...
\subsubsection{Simulation-Based \ac{PAKE}-Security}
...
While this might sound compelling, the research community never picked up on \ac{VPAKE}, where the server is not working with the password as input, but some other long-term secret related to the client's password \cite{Wu1998,Boyen2009a}.
...
\citet{Gentry2006} propose the first general technique, proven in the \ac{UC} framework, to make arbitrary \ac{PAKE} protocols resilient to server compromise.
...
Another reason for low adoption rate of \ac{VPAKE} in the research community could be its questionable benefit.
