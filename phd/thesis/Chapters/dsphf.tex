...
A \ac{D-SPHF} protocol between $n$ participants $P_0,\dots,P_x$ computing $h_x$ and $h_0$ consists of three interactive protocols \Setup, $\PHash_x^D$ and $\Hash_0^D$.
...
\mynote{unify $\Send(A, B m)$ for $A\rightarrow B$ or $B\rightarrow A$}
...
\end{tikzpicture}
\caption{Cramer-Shoup \ac{D-SPHF}\\{\small Dashed lines denote broadcast messages.}}
\label{fig:dsphf}
\end{figure}
...
\end{tikzpicture}
\caption{Two-Server PAKE framework using \ac{D-SPHF}
\\{\small Dashed lines denote broadcast messages.}}
\label{fig:twopake}
\end{figure}

\subsubsection{Security Analysis}
We use the well-known game based \ac{PAKE} model first introduced by \citet{Bellare2000} in it's two-server variant by \citet{Katz2012a}.
...
Let $(\HKGen,\allowbreak\PKGen,\allowbreak\PHash_0,\allowbreak\Hash_x,\allowbreak\Hash_0^D,\allowbreak\PHash_x^D)$ be a secure \ac{D-SPHF} and $(\KGen$, $\Enc$, $\Dec)$ a \ac{CCA}-secure labelled encryption scheme, then the proposed framework in Figure \ref{fig:twopake} is a secure \ac{2PAKE} protocol.
\end{theorem}
...
% 	
% 	\item $h\algout\PHash(\hp,\Laux,C,r):$
