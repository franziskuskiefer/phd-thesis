...
\begin{frame}{Zero-Knowledge Password Policy Checks 2/2}
  ...
  \begin{align*}    
    ...
    & \pi_i\in\Omega ~ \wedge \\
    & \sum_i^n C_i=\mathtt{Com}(\pi, \sum_i^n r_i) ~ \wedge \\
    & H\gets\PHash(\pi, r)\}
  \end{align*}
...
\begin{frame}{Security Model}
    \begin{description}
      ...
      \item[$\{(C,S_{1-b},s_b)\}\gets\mathsf{Corrupt}(S_b)$:] \hfill \\ simulates a server breach
    \end{description}
\end{frame}

\begin{frame}{Security Model}
  \begin{definition}[Policy Compliance]\label{def:pc}
  Policy Compliance of a 2BPR protocol holds if for every PPT adversary $A$ with access to $\mathsf{Setup}$ and $\mathsf{Send}_C$ oracles the probability that two server instances $S_{b,i}$ and $S_{1-b,j}$ exist after $A$ stopped that accepted $(C,S_{1-b},s_b)$, $(C,S_{b},s_{1-b})$ respectively, with $f(s_b+s_{1-b})=\mathtt{false}$ is negligible.
  \end{definition}
\end{frame}
...
%   The Password Blindness property of a Two-Server BPR protocol $\Pi$ holds if for every PPT adversary $A$ there exists a negligible function $\varepsilon(\cdot)$ such that
  \[ \Adv_{\Pi, A}^{\mathrm{PB}}=\left|\Pr[\Exp_{\Pi, A}^{\mathrm{PB}}=1]-\frac{1}{2}\right|\leq\varepsilon(\secpar). \]

  \structure{$\Exp_{\Pi, A}^{\mathrm{PB}}$}
  \begin{itemize}
    \item $(C_0, C_1, S_0, S_1,\pwd_0,\pwd_1)$ output by $A$ with access to $\mathsf{Setup},\mathsf{Send}_S,\mathsf{Send}_{SS}, \mathsf{Execute},\mathsf{Corrupt}$ $(\secpar,D,\{C\},\{S\})$ 
%     \item check $\pwd_0,\pwd_1\in D_{f_0\cap f_1}$, $|\pwd_0|=|\pwd_1|$, $C_0,C_1\in\{C\}$ and $S_0,S_1\in\{S\}$\\
...
%   \noindent
%   $\mathsf{Setup}'$ (in contrast to $\mathsf{Setup}$) uses $\pwd_b$ for client $C_0$ and $\pwd_{1-b}$ for $C_1$ with $b\rin\{0,1\}$ instead of choosing a random password or using the provided one.
  \end{definition}
\end{frame}
...
\begin{frame}{UC Security for 2Server PAKE}
  ...
  \begin{itemize}
    ...
    \item trapdoor SPHF introduced in \cite{Benhamouda??}
    \item necessary for efficient UC-secure protocol
