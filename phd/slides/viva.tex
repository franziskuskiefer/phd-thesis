...
% \end{frame}
%
\begin{frame}{Password Authenticated Key Exchange}{PAKE}
...
\begin{frame}{Security Model}
  ...
  Policy Compliance of a 2BPR protocol holds if for every PPT adversary $A$ with access to $\mathsf{Setup}$ and $\mathsf{Send}_C$ oracles the probability that two server instances $S_{b,i}$ and $S_{1-b,j}$ exist after $A$ stopped that accepted $(C,S_{1-b},\share_b)$, $(C,S_{b},\share_{1-b})$ respectively, with $f(\share_b+\share_{1-b})=\mathtt{false}$ is negligible.
  \end{definition}
\end{frame}
...
%   The Password Blindness property of a Two-Server BPR protocol $\Pi$ holds if for every PPT adversary $A$ there exists a negligible function $\varepsilon(\cdot)$ such that
  ...
  \begin{itemize}
    \item $(C_0, C_1, S_0, S_1,\share_0,\share_1)$ output by $A$ with access to $\mathsf{Setup},\mathsf{Send}_S,\mathsf{Send}_{SS}, \mathsf{Execute},\mathsf{Corrupt}$ $(\secpar,D,\{C\},\{S\})$ 
...
\begin{frame}{UC Security for 2Server PAKE}
  ...
  \begin{itemize}
    ...
    \item trapdoor SPHF introduced in \cite{Benhamouda2013}
