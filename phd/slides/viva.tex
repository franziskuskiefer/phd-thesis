...
%
% \begin{itemize}
% 	\item Password over HTML has many shortcomings
...
\begin{tikzpicture}[nodes = {font=\sffamily}]
  \foreach \percent/\name in {
      10/\textbf{d}igits,
      46/\textbf{s}ymbols,
      22/\textbf{u}pper case,
      22/\textbf{l}ower case,
    } {
...
%         \node at (\angle+0.5*\percent*3.6:0.7*\radius) {\name};
      ...
      \fi
      \node at (0,0) {$\Sigma$ (ASCII)};
    };
\end{tikzpicture}}

$\pwd=c_1 || \cdots || c_n\in\Sigma^n$
...
% \hspace*{1em} $\pi_i\gets\ichrint(c_i, i)$ \\
% \hspace*{1em} $r_i\rin\SSS_C; C_i\gets\Com(\pi_i;r_i)$ \\
% $H\gets \HashP(\pi;(s_P, s_H))$\\\hfill\\
% $\longrightarrow H,~\{C_i\}$
% \vspace*{1em}
% }
% \invblock<2->
%
...
%   \structure{Proofs}
% \end{center}
% $\forall i:~\SMP(\pi_i,r_i,\Omega')$ \\\hfill\\
% $\ZKPoK\{(\pi, s_P, s_H, r):$ \\
% \hspace*{1em} $H=\HashP(\pi; (s_P, s_H))$ \\
% \hspace*{1em} $\wedge ~ C=\Com(\pi;r)\}$
% \vspace*{1em}
% }
% \end{dynblock}
%
...
for $i\in[0,n-1]$ & & \\
\hspace*{1em} let $c_i\in\pwd$ & & \\
\hspace*{1em} \alert{$\pi_i\gets\chrint(c_i)$} & & \\
\hspace*{1em} \alert{$C_i=g^{\pi_i}h^{r_i}; C'_i=C_ih^{r'_i}$} & & \\
\alert{shuffle $C'_i \gets C'_{k_i}$} & & \\
let $\bm C = \{C_i\}$, $\bm C' = \{C'_i\}$ & & \\
$s_P\gets_R\PPHSalt(\paramP)$ & & \\
$s_H\gets_R\PHSalt(\paramP)$ & & \\
$(H_1,H_2)\gets\HashP(\pi; (s_P, s_H))$ & & \\
For $c_i\in\pwd$ identify set $\omega_{k_i}$ & & \\
Execute \PoE, \PoM, \PoS: & \alert{$\bm C, \bm C', \vect{\omega}, H_1,H_2, s_H,$} & If $|\bm C|$ $=$ $|\bm C'|$ $\geq$ $\pmin$: \\
...
\begin{frame}{A Different Approach 3/3}
  ...
  \structure{Security:} New simulation based security model
  \begin{itemize}
...
\begin{frame}{Verifier-based PAKE}{How NOT to do it}
  ...
  \begin{center}    
  \structure{This is a \emph{PAKE} not a \emph{VPAKE}}
  \end{center}
\end{frame}
...
\begin{frame}{Two-Server Password Authentication Framework}{2BPR}
  \begin{figure}
  \centering
...
\begin{frame}{Modelling Passwords and Policies}
  
\def\firstcircle{(0,0) circle (1.5cm)}
\def\secondcircle{(45:2cm) circle (1.5cm)}

% Now we can draw the sets:
\makebox[\textwidth][c]{\begin{tikzpicture}[overlay, remember picture]
    % The last trick is to cheat and use transparency
    \begin{scope}[shift={(2cm,-2cm)}, fill opacity=0.5]
        \fill[red] \firstcircle;
        \fill[green] \secondcircle;
        \draw \firstcircle node[below] {$f_1$};
        \draw \secondcircle node [above] {$f_2$};
        \node at (0.7,0.7) {$f$};
    \end{scope}
\end{tikzpicture}}
...
\begin{frame}{Blind Password Registration for Two Servers}
  \structure{The Proof $(\pwd, \share_0, \share_1)$}
  \[
...
\begin{frame}{Distributed Smooth Projective Hashing}
...
\begin{beamerboxesrounded}[upper=uppercol,lower=lowercol,shadow=true]{\centering Idea}
	Encrypt $\pwd$, $\share_1$ and $\share_2$ $\rightarrow$ Use Distributed SPHF to compute keys
\end{beamerboxesrounded}
...
\begin{frame}{2-Server PAKE from DSPHF}
		...
		\begin{tikzpicture}
			...
			\draw[pil,<-] ($(client.west)+(1.3,0.4)$) -- node[above=0.5cm,anchor=center,rotate=18]{$k_{p,1},C_1$} (server1);
			\draw[pil,<-] ($(client.west)+(1.3,-0.4)$) -- node[below=0.5cm,anchor=center,rotate=-18]{$k_{p,2},C_2$} (server2);
		\end{tikzpicture}
		\end{figure}
...
\begin{frame}{UC Security for 2Server PAKE}
  ...
  \begin{itemize}
    \item \structure{trapdoor SPHF} introduced by \cite{Benhamouda2013}
    \item necessary for \emph{efficient} UC-secure protocol
    \item \structure{$\mathsf{THash}$} can compute hash without hashing key, uses trapdoor instead
    \item \structure{$\mathsf{VerKp}$} verifies that $k_p$ is a valid projection key
...
\begin{itemize}\itemsep1em
	...
	\item Implementation and demo of BPR frameworks
...
\begin{frame}{Password Authenticated Key Exchange}{SPAKE}
 ...
 \cite{Abdalla2005}: SPAKE uses DL-hard group $G$ and public $M,N\in G$; random oracle $H$
...
\begin{frame}{Password Hashing}
  \begin{center}
    \structure{$H\gets\mathtt{Hash}(\pwd, s_P, s_H)$} %$=\text{PHash}(p, P, s_P, s_H)$ with $P\gets\text{PPreHash}(p, \pwd, s_P)$
  \end{center}
%   \begin{tabular}{ll}
...
\begin{frame}{Secure Set-based Policy Checking}
  ...
  \begin{itemize}
    \item \alert{Client with set $\mathcal{I}$ gets $s_i$ iff $i\in\mathcal{I}\cap\mathcal{V}$}
  \end{itemize}
\end{frame}
...
\section{UC 2PAKE Backup}

\begin{frame}{UC 2PAKE Functionality}{KEX Init$_\Client$}
