...
\begin{frame}{Password Authenticated Key Exchange}{PAKE 1/2}
	...
	\begin{beamerboxesrounded}[upper=uppercol,lower=lowercol,shadow=true]{\centering Passwords}\centering
		Common secret can have only limmited entropy
...
\begin{frame}{Forward-Secrecy in PAKE 2/4}{}

	Different corruption models in PAKE literature
	\begin{itemize}
		\item\emph{Zero:} $\mathcal{A}$ has no access to the $\mathsf{Corrupt}$ oracle.
		\item\emph{Static:} $\mathcal{A}$ has no access to the $\mathsf{Corrupt}$ oracle but can initially retrieve passwords $\pwd_{C,S}$ before the experiment starts.
		\item\emph{Adaptive:} $\mathcal{A}$ has access to the $\mathsf{Corrupt}(C,S)$ oracle to retrieve $\pwd_{(C,S)}$.
		\item\emph{Strong:} $\mathcal{A}$ has access to the $\mathsf{Corrupt}(C,S)$ oracle to retrieve state information of all client instances $C_i$ that have been invoked with server $S$, including $\pwd_{C,S}$.
	\end{itemize}
...
\begin{frame}{Forward-Secrecy in PAKE 3/4}{}
	...
	\begin{dynblock}
		...
		\opaqueblock<1>{
				\begin{quote}
					``[The static corruption model is unnecessary in PAKE as] the independence among the passwords shared between pairs of honest participants and those shared with malicious users.''
				\end{quote}
				\vskip5mm
...
\begin{itemize}
	...
	\item $\mathsf{Hash}(k_h,L_x,C)$ outputs the hash value $h=\Theta_x(C)\odot k_h\in\GG$.
	\item $\mathsf{PHash}(k_p,L_x,C,w)$ returns the hash value $h=\lambda\odot k_p\in\GG$, with $\lambda=\Omega(w)$.
\end{itemize}
}
\invblock<2->
...
\opaqueblock<2>{\hspace*{1em}
	\begin{itemize}
		\item \emph{Correctness}: If $C\in L$, with $w$ proving so, then $\mathsf{Hash}(k_h,L_x,C)=\mathsf{PHash}(k_p,L_x,C,w)$.
		\item \emph{Smoothness}: If $C\not\in L_x$, the hash value $h$ is statistically indistinguishable from a random element from $\GG$.
		\item \emph{Pseudorandomness}: If $C\in L_x$, the hash value $h$ is indistinguishable from a random element from $\GG$.
	\end{itemize}
}
\end{dynblock}
