...
\fk[inline]{some papers...}
\fk[inline]{google authenticator etc.}
...
\fk[inline]{Some words on PPSS}

\subsection{Password Based Encryption}
\fk[inline]{some words on PBE}
...
This work consists of five chapters and an appendix.
We gave a motivational overview and discussion on low-entropy cryptography and passwords already in this introduction.
...
The main contribution of this work is given in Chapter \ref{ch:pake} and \ref{ch:malservers}.
The former chapter summarises the contribution in the field of \acl{PAKE} on \ac{PAKE} security models by recalling the high-level intuitions from Appendix \ref{paper:opake} on oblivious \ac{PAKE} and Appendix \ref{paper:corrupt} on forward-secrecy and corruption in \ac{PAKE} models.
...
It compiles intuitions from Appendix \ref{paper:2pake} on \aclp{SPHF} and two-server \ac{PAKE} as well as an outlook on just started work on blind password policy checking.

\fk[inline]{ToDo --- Motivation !!!!}
