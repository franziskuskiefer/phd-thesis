...
For a insightful and skeptical discussion on this kind of security verification we refer to the \myhref{http://anotherlook.ca/}{``Another Look''} series by Koblitz and Menezes.
...
A formal view on adversarial behaviour and how to model security protocols goes back to works from \citeauthor{Needham78} \citeyear{Needham78} \cite{Needham78} and \citeauthor{Dolev83} \citeyear{Dolev83} \cite{Dolev83}.
The Dolev-Yao model models cryptographic protocols by assuming black-box cryptographic primitives and an adversary that is able to control the entire communication channel between the participating parties.
...
The computational world consists of bitstring messages and cryptographic primitives calculating on them.
...
Game-based security models allow the \ac{PPT} adversary plays a game against a challenger.
...
On (possibly empty) input by the adversary they return the result of their computation to him.

There are different commonly used ways of modelling security.
However, all \ac{PAKE} model aim at \ac{AKE}-security.
We describe only the ones used here.
...
The notion of \ac{RoR} in the context of \ac{AKE} protocols has been introduced by \citeauthor{Abdalla2005} to strengthen and simplify the \ac{FtG} approach used in the original model \cite{Bellare2000} towards \ac{RoR}.
...
The idea of simulation-based security proofs are due to \citeauthor{Goldreich87} \citeyear{Goldreich87} \cite{Goldreich87}.
\fk[inline]{General}
\fk[inline]{UC}
